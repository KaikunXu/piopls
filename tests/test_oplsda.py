...
def test_features_df_and_vip_ranking(dummy_data):
    """Test feature output table, verifying VIP ranking and updated variable name extraction."""
    df_X, df_y = dummy_data
    model = OPLSDA(n_ortho=1, cv_folds=3)
    model.fit(df_X, df_y)
...
def test_scores_df_metadata(dummy_data):
    """Test sample score table, verifying sample name memorization and fitted values."""
    df_X, df_y = dummy_data
    model = OPLSDA(n_ortho=1, cv_folds=3)
    model.fit(df_X, df_y)
    ...
    # Verify the new consistency check column
...
def test_sacurine_example_data(tmp_path):
    ...
    # Test new global info DataFrame
    df_info = model.get_model_info_df()
